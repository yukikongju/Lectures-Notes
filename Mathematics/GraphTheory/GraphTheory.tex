...
\begin{theorem}[Every connected graph has a spanning tree]
...
\begin{definition}[Non-Separable Graph]
...
\begin{theorem}[Caracterisation of a Non-sperable Graph]
...
\begin{theorem}[Cut Vertices are $deg(v) \geq 2$]
...
% \begin{theorem}[A cut vertex v lies on every uv path in G-v]
    % Let v be a cut vertex of a connected graph G. Then, G-v is disconnected
    % and has 2 components. Let u and w be vertices from the distincts components
    % of G-v. Then, v lies on every u-w path of G-v
% \end{theorem}

\begin{theorem}[Caracterisation of Cut Vertices]
...
\begin{lemma}[Every non-trivial graph contains two vertices that are not cut
    vertices]
\end{lemma}

\begin{theorem}[The fartest vertex from another vertex is not a cut vertex]
...
\begin{theorem}[Bound on vertex Connectivity]
...
\begin{remark}[Show that graph is k-connected]
...
\begin{proposition}[Petersen Graph and Vertex Transitivity]
...
\begin{definition}[Vertex Separating Sets]
...
\begin{remark}[A vertex separating set is not unique]
\end{remark}
...
\begin{definition}[Vertex Disjoints Paths]
...
\begin{theorem}[If two paths are vertex disjoints, then they are edges
    disjoints]
\end{theorem}
